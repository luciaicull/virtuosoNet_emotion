...
num_tempo_info = 0
# distance from marking, dynamics vector 4, mean_piano, forte marking and velocity = 4
num_dynamic_info = 0
...
def cal_tempo_loss_in_beat(pred_x, true_x, note_locations, start_index):
            ...
            if 'baseline' in args.modelCode:
                for j in range(i, num_notes):
                    if note_locations[j+start_index].beat > current_beat:
                        break
                if not i == j:
                    pred_beat_tempo[current_beat -
                                    start_beat] = torch.mean(pred_x[0, i:j, QPM_INDEX])
                    true_beat_tempo[current_beat -
                                    start_beat] = torch.mean(true_x[0, i:j, QPM_INDEX])
            else:
                pred_beat_tempo[current_beat-start_beat] = pred_x[0,
                                                                  i, QPM_INDEX:QPM_INDEX + NUM_TEMPO_PARAM]
                true_beat_tempo[current_beat-start_beat] = true_x[0,
                                                                  i, QPM_INDEX:QPM_INDEX + NUM_TEMPO_PARAM]
    ...
    if args.deltaLoss and pred_beat_tempo.shape[0] > 1:
        ...
        tempo_loss = (tempo_loss + delta_loss * DELTA_WEIGHT) / \
            (1 + DELTA_WEIGHT)
...
if args.sessMode == 'train':
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if RAND_TRAIN:
            ...
            while len(remaining_samples) > 0:
                ...
                if selected_sample.slice_indexes is None:
                    ...
                    if HIER_MEAS and HIERARCHY:
                        ...
                        selected_sample.slice_indexes = dp.make_slicing_indexes_by_measure(
                ...
                for i in range(num_key_augmentation+1):
                    ...
                    kld_weight = sigmoid(
                        (NUM_UPDATED - KLD_SIG) / (KLD_SIG/10)) * KLD_MAX
            ...
            for xy_tuple in train_xy:
                ...
                total_batch_num = int(
                    math.ceil(data_size / (TIME_STEPS * batch_size)))
        ...
        for xy_tuple in test_xy:
            ...
            align_matched = torch.Tensor(
                align_matched).view(1, -1, 1).to(DEVICE)
            ...
            if HIERARCHY:
                ...
                vel_y = MODEL.note_tempo_infos_to_beat(
                    batch_y, hierarchy_numbers, 0, 1)

                tempo_loss = criterion(outputs[:, :, 0:1], tempo_y)
                vel_loss = criterion(outputs[:, :, 1:2], vel_y)
                if args.deltaLoss:
                    tempo_out_delta = outputs[:, 1:,
                                              0:1] - outputs[:, :-1, 0:1]
                    tempo_true_delta = tempo_y[:, 1:, :] - tempo_y[:, :-1, :]
                    vel_out_delta = outputs[:, 1:, 1:2] - outputs[:, :-1, 1:2]
                    vel_true_delta = vel_y[:, 1:, :] - vel_y[:, :-1, :]

                    tempo_loss += criterion(tempo_out_delta,
                                            tempo_true_delta) * DELTA_WEIGHT
                    vel_loss += criterion(vel_out_delta,
                                          vel_true_delta) * DELTA_WEIGHT
                ...
                tempo_loss = cal_tempo_loss_in_beat(
                    outputs, batch_y, note_locations, 0)
                if LOSS_TYPE == 'CE':
                    vel_loss = criterion(outputs[:, :, NUM_TEMPO_PARAM:NUM_TEMPO_PARAM+len(
                        BINS[1])], batch_y[:, :, NUM_TEMPO_PARAM:NUM_TEMPO_PARAM+len(BINS[1])], align_matched)
                    ...
                    vel_loss = criterion(
                        outputs[:, :, VEL_PARAM_IDX], batch_y[:, :, VEL_PARAM_IDX], align_matched)
                    deviation_loss = criterion(
                        outputs[:, :, DEV_PARAM_IDX], batch_y[:, :, DEV_PARAM_IDX], align_matched)
                    articul_loss = criterion(
                        outputs[:, :, PEDAL_PARAM_IDX], batch_y[:, :, PEDAL_PARAM_IDX], pedal_status)
                    pedal_loss = criterion(
                        outputs[:, :, PEDAL_PARAM_IDX+1:], batch_y[:, :, PEDAL_PARAM_IDX+1:], align_matched)
                    trill_loss = torch.zeros(1)
                for z in total_z:
                    perform_mu, perform_var = z
                    kld_loss = -0.5 * \
        ...
        mean_valid_loss = (mean_tempo_loss + mean_vel_loss + mean_deviation_loss +
                           mean_articul_loss + mean_pedal_loss * 7 + mean_kld_loss * kld_weight) / (11 + kld_weight)
    ...
    if os.path.isfile('prime_' + args.modelCode + args.resume):
        ...
        if IN_HIER:
            ...
            HIER_MODEL = nnModel.HAN_Integrated(
                HIER_MODEL_PARAM, DEVICE, True).to(DEVICE)
    ...
    if args.sessMode == 'test':
        ...
        for xy_tuple in complete_xy:
            ...
            if piece_changed or current_perf_index == len(complete_xy)-1:
                prev_perf_x = test_x
                if piece_wise_loss:
                    piece_wise_loss_mean = np.mean(
                        np.asarray(piece_wise_loss), axis=0)
            ...
            predicted_perf_worm_data = perf_worm.cal_tempo_and_velocity_by_beat(output_as_feature, note_locations,
                                                                                momentum=0.2)
            zero_prediction_as_feature = zero_prediction.view(
                -1, NUM_OUTPUT).cpu().numpy()
            ...
            if LOSS_TYPE == 'CE':
                ...
                deviation_loss = criterion(
                    outputs[:, :, NUM_TEMPO_PARAM +
                            len(BINS[1]):NUM_TEMPO_PARAM + len(BINS[1]) + len(BINS[2])],
                    batch_y[:, :, NUM_TEMPO_PARAM + len(BINS[1]):NUM_TEMPO_PARAM + len(BINS[1]) + len(BINS[2])])
                pedal_loss = criterion(
                    outputs[:, :, NUM_TEMPO_PARAM +
                            len(BINS[1]) + len(BINS[2]):-num_trill_param],
                    batch_y[:, :, NUM_TEMPO_PARAM + len(BINS[1]) + len(BINS[2]):-num_trill_param])
                trill_loss = criterion(
                    outputs[:, :, -num_trill_param:], batch_y[:, :, -num_trill_param:])
            else:
            ...
            for z in perform_z:
                perform_mu, perform_var = z
                kld = -0.5 * torch.sum(1 + perform_var -
                                       perform_mu.pow(2) - perform_var.exp())
                piece_kld.append(kld)
            piece_kld = torch.mean(torch.stack(piece_kld))

            piece_wise_loss.append((tempo_loss.item(), vel_loss.item(), deviation_loss.item(
            ), articul_loss.item(), pedal_loss.item(), trill_loss.item(), piece_kld.item()))
        ...
        mean_valid_loss = (mean_tempo_loss + mean_vel_loss +
                           mean_deviation_loss / 2 + mean_pedal_loss * 8) / 10.5
        ...
        for piece_corr in selected_corr:
            ...
            for slice_corr in piece_corr:
                ...
                for i in range(slice_corr.num_performance):
                    tempo_r, _ = xml_matching.cal_correlation(
                        tempos, human_tempos[i])
                    dynamic_r, _ = xml_matching.cal_correlation(
                        dynamics, human_dynamics[i])
                    model_correlation_results._append_result(
                        tempo_r, dynamic_r)
                ...
                save_name = 'test_plot/' + path.replace('chopin_cleaned/', '').replace(
                    '/', '_', 10) + '_note{}-{}_by_{}.png'.format(slc_idx[0], slc_idx[1], args.modelCode)
