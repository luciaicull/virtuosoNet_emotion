...
def batch_train_run(data, model, args, optimizer):
    ...
    if model.config.hierarchy_level in ['measure', 'beat'] and not model.config.is_dependent: # han_measure_ar
...
def handle_data_in_tensor(x, y, model_config, device, hierarchy_test=False):
    ...
    if model_config.hierarchy_level in ['measure', 'beat'] and not model_config.is_dependent: # han_measure_ar
        y = hierarchy_output
    elif model_config.is_dependent: # han_note_ar
